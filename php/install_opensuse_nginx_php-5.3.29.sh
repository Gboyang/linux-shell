...
make install
cd ..
cp ./php-5.3.29/php.ini-production /server/php/etc/php.ini
#adjust php.ini
sed -i 's#; extension_dir = \"\.\/\"#extension_dir = "/server/php/lib/php/extensions/no-debug-non-zts-20090626/"#'  /server/php/etc/php.ini
...
./configure -with-openssl -with-php-config=/server/php/bin/php-config
make && make install
echo 'extension = "openssl.so"' >>  /server/php/etc/php.ini
